...
void PluginEditor::paintPadBox(juce::Graphics& g, juce::Rectangle<int> box, int padIndex)
{
    ...
    // Pitch/stretch indicators (top-right corner of pad)
    float st = slot.getPitchSemitones();
    float ts = slot.getTimeStretch();
    juce::String warpInfo;
    if (st != 0.0f) warpInfo += (st > 0 ? "+" : "") + juce::String(st, 0) + "st";
    if (ts < 0.99f || ts > 1.01f) {
        if (warpInfo.isNotEmpty()) warpInfo += " ";
        warpInfo += juce::String(ts, 1) + "x";
    }
    if (warpInfo.isNotEmpty()) {
        g.setColour(juce::Colour(0xFF42A5F5));
        g.setFont(11.0f);
        g.drawText(warpInfo, box.getX() + 8, box.getY() + 4, box.getWidth() - 16, 14,
...
void PluginEditor::paintFileBrowser(juce::Graphics& g, juce::Rectangle<int> area)
{
    ...
    g.setColour(juce::Colour(kTabActive));
    g.setFont(16.0f);
    juce::String header = browseCurrentDir_.getFileName().isEmpty() ? "/" : browseCurrentDir_.getFileName();
    g.drawText(header, content.removeFromTop(24), juce::Justification::centredLeft);

    // Target pad indicator
    g.setColour(juce::Colour(kEncLabel));
    g.setFont(13.0f);
    g.drawText("-> PAD " + juce::String(selectedPad_ + 1),
               content.removeFromTop(18), juce::Justification::centredLeft);
    ...
    for (int i = 0; i < visibleRows; ++i)
    {
        ...
        if (!isDir && idx < browseItemDurations_.size() && browseItemDurations_[idx].isNotEmpty()) {
            g.setColour(sel ? juce::Colour(0xCCFFFFFF) : juce::Colour(0xFF666666));
            g.setFont(12.0f);
            g.drawText(browseItemDurations_[idx], row.getX(), row.getY(), row.getWidth() - 8, rowH,
...
void PluginEditor::enterBrowseMode()
{
    ...
    encoderSlots_[3].nameLabel.setText("STAR", juce::dontSendNotification);
    encoderSlots_[3].valueLabel.setText("todo", juce::dontSendNotification);
    repaint();
}
...
void PluginEditor::browseScanCurrentDir()
{
    ...
    for (auto& f : files) {
        ...
        if (auto* reader = browseFormatMgr_.createReaderFor(f)) {
            ...
            if (secs < 1.0)
                dur = juce::String((int)(secs * 1000)) + "ms";
            else if (secs < 60.0)
                dur = juce::String(secs, 1) + "s";
            else
                dur = juce::String((int)(secs / 60)) + ":" + juce::String((int)secs % 60).paddedLeft('0', 2);
            delete reader;
        }
        browseItemDurations_.add(dur);
    }
...
void PluginEditor::onButton(int n, bool val)
{
    ...
    if (browseMode_) {
        // In browse mode: just select pad target, don't trigger
        selectedPad_ = n;
        repaint();
        return;
    }
    // Normal mode: trigger + select
