...
void PluginEditor::resized()
{
    ...
    static constexpr int nudge[] = { -14, -8, 14, 20 };
...
void PluginEditor::paintPadBox(juce::Graphics& g, juce::Rectangle<int> box, int padIndex)
{
    ...
    // Pitch/stretch indicators (on top of waveform)
    ...
    if (warpInfo.isNotEmpty()) {
        g.setColour(juce::Colour(0xFF42A5F5));
        g.setFont(12.0f);
        g.drawText(warpInfo, box.getX() + 8, box.getBottom() - 18, box.getWidth() - 16, 14,
...
void PluginEditor::onButton(int n, bool val)
{
    ...
    // Buttons ALWAYS trigger pads. Same muscle memory everywhere.
    auto& slot = processor_.getEngine().getSlot(n);
    // If pad is looping and playing, stop it. Otherwise trigger.
    if (slot.isPlaying() && (slot.getMode() == PadMode::Loop || slot.getMode() == PadMode::ClockedLoop)) {
        slot.stop();
    } else {
...
void PluginEditor::onEncoder(int n, float delta)
{
    ...
    switch (currentPage_)
    {
        ...
        case PAGE_FADE: {
            // Accelerate: 5ms steps under 500, 25ms steps above
            float fadeStep = 5.0f;
            if (n == 1) {
                if (slot.getFadeInMs() > 500.0f) fadeStep = 25.0f;
                slot.setFadeInMs(slot.getFadeInMs() + delta * fadeStep);
            }
            if (n == 2) {
                if (slot.getFadeOutMs() > 500.0f) fadeStep = 25.0f;
                slot.setFadeOutMs(slot.getFadeOutMs() - delta * fadeStep);  // opposite direction
            }
            break;
        }
    }
